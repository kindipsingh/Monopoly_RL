...
def train_agent(num_games=2, save_interval=1, learning_rate=1e-5, gamma=0.99,
    ...
    for game_num in tqdm(range(num_games), desc="Training Games"):
        ...
        logger.info(f"Epsilon: {ddqn_agent_instance.ddqn_agent.epsilon:.4f}, Replay buffer size: {len(ddqn_agent_instance.ddqn_agent.replay_buffer.buffer)}")
    ...
    logger.info(f"  Replay buffer size: {len(ddqn_agent_instance.ddqn_agent.replay_buffer.buffer)}")
