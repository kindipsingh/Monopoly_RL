...
def mortgage_property(player, asset, current_gameboard):
    ...
    """
    if type(asset) == str:
        logger.debug("The asset is a string, cannot manipulate mortgage on it.")
        return flag_config_dict['failure_code']
    elif asset.owned_by != player:
...
def improve_property(player, asset, current_gameboard, add_house=True, add_hotel=False):
    ...
    """
    if type(asset) == str:
        logger.debug("The asset is a string, cannot improve property on it.")
        return flag_config_dict['failure_code']
    elif asset.owned_by != player or asset.is_mortgaged:
...
def improve_property_reverse_improvement(player, asset, current_gameboard, add_house=True, add_hotel=False):
    """
    Function for improving asset belonging to player by adding house or hotel.
    Another thing to remember is that once you add a hotel, it stands as its own unit. If you decide to sell, you'll
    have to sell the entire hotel or not at all.
    ...
    if add_hotel: # this is the simpler case
        ...
        if asset.num_hotels == 0:
            # ----------------------novelty----------------------------
            logger.debug('You need to have 1 hotel before you can build a house...Returning failure code')
            # ---------------------------------------------------------
            return flag_config_dict['failure_code']
        flag = True
        current_asset_num_houses = asset.num_houses
...
def post_roll_arbitrary_action(from_player=None, to_player=None, action_params_dict=None, current_gameboard=None):
    ...
    Example:
        ...
        or railroad, also Player_4 is physically in the location position, and the location is owned by bank. We then need to
...
def accept_arbitrary_interaction(from_player, to_player, interaction_id, decision, current_gameboard):
    ...
    if from_player == to_player:
        logger.debug(str(to_player) + ' cannot accept/reject offer from itself')
        return flag_config_dict['failure_code']
    current_gameboard['history']['function'].append(to_player.make_out_of_turn_moves)
    params = dict()
    detail_params = dict()
    detail_params['interaction_id'] = interaction_id
    detail_params['from_player'] = from_player.player_name
    detail_params['to_player'] = to_player.player_name
    detail_params['location'] = from_player.agent._agent_memory['free_rent'][interaction_id]['location'].name
    detail_params['to_location'] = from_player.agent._agent_memory['free_rent'][interaction_id]['to_location'].name
    detail_params['decision'] = decision
    params['interactions'] = detail_params
    if decision:
        params['description'] = 'accept free rent interaction offer'
    else:
        params['description'] = 'reject free rent interaction offer'
    current_gameboard['history']['param'].append(params)
    current_gameboard['history']['return'].append(None)
    current_gameboard['history']['time_step'].append(current_gameboard['time_step_indicator'])
    ...
    if decision:  # if accept the offer
        ...
        logger.debug(
            str(to_player.player_name) + ' accepts the offer from ' + str(from_player.player_name) + '. When ' + str(to_player.player_name) +
            'passes ' + str(from_player.agent._agent_memory['free_rent'][interaction_id]['location'].name) + ' or ' +
            str(from_player.player_name) + ' passes ' + str(from_player.agent._agent_memory['free_rent'][interaction_id]['to_location'].name) + ',' +
    ...
    if not decision:  # if to_player reject this offer, from_player delete corresponding offer from its memory
        logger.debug(
            'The offer from ' + str(from_player.player_name) + ' to ' + str(to_player.player_name) + ' is refused by ' + str(to_player.player_name) + '.')
...
def print_pre_roll_arbitrary_action_schema(current_gameboard):
    print('print_pre_roll_arbitrary_action_schema')
    # check location is a real location:
    # if not return -1
    if 'pre_roll_arbitrary_action_schema' in current_gameboard:
        current_gameboard['schema'] = current_gameboard['pre_roll_arbitrary_action_schema']
    return flag_config_dict['successful_action']
...
def print_post_roll_arbitrary_action_schema(current_gameboard):
    print('print_post_roll_arbitrary_action_schema')
    if 'post_roll_arbitrary_action_schema' in current_gameboard:
        current_gameboard['schema'] = current_gameboard['post_roll_arbitrary_action_schema']
    return flag_config_dict['successful_action']
        #return {'location': 'Location', 'num_rounds': 'Rounds', 'installment_interest': 'Perc_of_amount','down_payment': 'Amount'}


def print_out_of_turn_arbitrary_action_schema(current_gameboard):
    print('print_out_of_turn_arbitrary_action_schema')
    if 'out_of_turn_roll_arbitrary_action_schema' in current_gameboard:
        current_gameboard['schema'] = current_gameboard['out_of_turn_roll_arbitrary_action_schema']
    return flag_config_dict['successful_action']


def print_interaction_arbitrary_action_schema(current_gameboard):
    print('print_interaction_arbitrary_action_schema')
    if 'interaction_schema' in current_gameboard:
        current_gameboard['schema'] = current_gameboard['interaction_schema']
    return flag_config_dict['successful_action']
