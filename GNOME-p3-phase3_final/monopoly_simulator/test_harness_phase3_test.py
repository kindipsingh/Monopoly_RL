...
from monopoly_simulator import gameplay_socket_phase3 as gameplay
from monopoly_simulator.monopoly_state_encoder import MonopolyStateEncoder, MonopolyStateVisualizer
from monopoly_simulator.logging_info import log_file_create
...
def log_game_state(game_elements):
    """
    Callback function that encodes the current game state into a 240-dimensional vector
    and prints the raw vector as well as a formatted visualization.
    """
    print("STATE LOGGER CALLED")  # Debug print to verify callback is hit
    sys.stdout.flush()

    encoder = MonopolyStateEncoder()
    visualizer = MonopolyStateVisualizer(encoder)
    game_elements_copy = copy.deepcopy(game_elements)
    encoded_state = encoder.encode_state(game_elements_copy)
    print("\n----- Encoded Game State (240D Vector) -----")
    print(encoded_state.squeeze().numpy())
    sys.stdout.flush()

    visualizer.visualize_state(encoded_state)
    visualizer.decode_state(encoded_state)
    sys.stdout.flush()

def play_tournament_with_novelty_v3(agent_combination_1, agent_combination_2, tournament_log_folder=None, nov=None, meta_seed=5, num_games=100, novelty_index=25, novelty_info=False, debug_mode=False, novelty_name=None):
    """
    Tournament logging is not currently supported, but will be soon.
    :param tournament_log_folder: String. The path to a folder.
    :param meta_seed: The seed we use to generate a sequence of seeds for games.
    :param num_games: The number of games to simulate.
    :return: None. Will print win-loss metrics and write out game logs.
    """
    if nov:
        print("novelty function is: " + nov)
    else:
        print("novelty function is: None (Goals)")
    print("novelty name is: " + str(novelty_name))
    ...
    for i in range(len(agent_combination_1)):
        ...
        for t in range(0, novelty_index):
            ...
            logger_inst = log_file_create(filename)

            if debug_mode:
                victor = gameplay.play_game_in_tournament_socket_phase3(
                    tournament_seeds[t],
                    agent_combination_1[i][0],
                    agent_combination_1[i][1],
                    agent_combination_1[i][2],
                    agent_combination_1[i][3],
                    novelty_info=novelty_info,
                    state_logger=log_game_state   # <-- Pass the state logger callback
                )
            else:
                try:
                    victor = gameplay.play_game_in_tournament_socket_phase3(
            ...
            handlers_copy = logger_inst.handlers[:]
...
def execute_tournament(novelty_name, tournament_idx, metaseed, novelty_index, novelty_info, agent_type, port, num_games, debug):
    ...
    play_tournament_with_novelty_v3(agent_combination_1, agent_combination_2, tournament_log_folder = f'/system_detection/tournament_with_novelty_trial_{novelty_name}_{tournament_idx}', nov=nov, meta_seed=metaseed, num_games=num_games, novelty_index=novelty_index, novelty_info=novelty_info, debug_mode = debug, novelty_name = novelty_name)
